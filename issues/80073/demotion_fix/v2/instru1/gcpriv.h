...
{
    gc_stage_gtc = 0,
    gc_stage_joined_gtc = 1,
    gc_stage_do_pre_gc = 2,
    gc_stage_mark = 3,
    gc_stage_plan = 4
};

enum bgc_creation_result
{
    bgcc_still_running = 0,
    bgcc_thread_exists = 1,
    bgcc_null_thread = 2,
    bgcc_creation_failed = 3
};
...
#define USE_PADDING_TAIL  2
    ...
    PER_HEAP
    BOOL prepare_bgc_thread(gc_heap* gh, int* new_thread_res);
    PER_HEAP
...
#define max_saved_ngc2_condemn_reasons_count 64
    ...
    struct failed_bgc_start_info
    {
        // how many us did it take for us to create the bgc thread
        uint64_t bgc_thread_creation_time;
        size_t gc_index;
        int bgc_creation_res;
        bool bgc_thread_created_p;
        // or was it due to failure to commit mark array?
        bool mark_array_committed_p;
    };

#define max_saved_failed_bgc_start_info_count 64
    PER_HEAP_ISOLATED
    failed_bgc_start_info saved_failed_bgc_start_info[max_saved_failed_bgc_start_info_count];
    PER_HEAP_ISOLATED
    int current_failed_bgc_start_info_index;
    PER_HEAP_ISOLATED
    void add_to_failed_bgc_start_info (uint64_t creation_time, int bgc_creation_res, bool bgc_thread_p, bool ma_committed_p);
...
#define max_saved_ngc2_index_count 12800
    PER_HEAP_ISOLATED
    ...
    PER_HEAP_ISOLATED
    void add_to_ngc2_index_info(int gen_num, gc_type type, gc_stage stage);
...
#define max_saved_gen_plan_info_count 8
...
#define max_saved_art_plug_count 8
